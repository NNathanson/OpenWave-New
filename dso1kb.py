...
class Dso:
    ...
    def connect(self, str):
        ...
        #print(self.read(self.write("*LRN?\n")))
        #print(self.write(":TRIGger:SOURce?\n"))
    ...
    def getRawData(self, header_on,  ch): #Used to get waveform's raw data.
        ...
        if(header_on == True):
            ...
            #dT = str(dT)
            #f = open('Raw_{0}_Ch{1}'.format(dT,ch),'wb')
            #f.write(rDat)
            #f.close()
...
#############
    ...
    def convertWaveform(self, ch, factor):
        ...
        if(factor==1):
            ...
            sec = dT.second
            msec = dT.microsecond
            dateStr = "{0}_{1}_{2} {3}_{4}_{5}_{6}".format(year,month,day,hour,min,sec,msec)
            
            f = open('Plottable_{0}_Ch{1}'.format(dateStr,ch),'w')
            f.write(str(fWave))
            f.close()
